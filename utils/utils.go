...
type UtilityService interface {
	...
	EncryptFileInChunksAndWriteInOutputFile(inputPath string, outputPath string, password string, maxChunkSize int) error
...
//EncryptFileInChunksAndWriteInOutputFile - buffered encryption and write it in the output file, mainly command line request to agent
func (Utils *UtilsService) EncryptFileInChunksAndWriteInOutputFile(inputPath string, outputPath string, password string, maxChunkSize int) error {
	defer func() {
		if r := recover(); r != nil {
	...
	totalFileSizeLeftToEncrypt := inputFileStat.Size()
	totalChunks := math.Ceil(float64(totalFileSizeLeftToEncrypt) / float64(maxChunkSize))
	outputFile, err := os.Create(outputPath)
	if err != nil {
		return err
	}
	for i := 1; i <= int(totalChunks); i++ {
		bufferSize := getBufferSize(totalFileSizeLeftToEncrypt, maxChunkSize)
		if bufferSize == 0 {
			break
		}
		...
		compressedChunk := Utils.Compress(buffer)
		encryptedData, err := Utils.EncryptData(compressedChunk, password)
		if err != nil {
			return err
		}

		size := Utils.get64BitBinaryStringNumber(int64(len(encryptedData)))
		_, err = outputFile.Write([]byte(size))
		if err != nil {
			return err
		}
		...
		encryptedData = nil
		totalFileSizeLeftToEncrypt -= bufferSize
		buffer = nil
	}
...
func CheckFileRegexAndExtension(fileName string, fileExtensions []models.FileExtensionStruct, fileNameRegexs []string) bool {
	matched := false
	matched1 := false
	matched2 := false
...
func (Utils *UtilsService) GetFileDetails(filePath string, AppFolder string, FlowName string, fileExtensions []models.FileExtensionStruct, fileNameRegexs []string) (flowName string, originalFileName string, originalAbsoluteFilePath string, newFileName string, newLocation string, md5CheckSum string, err error) {
	...
	matched = CheckFileRegexAndExtension(originalFileName, fileExtensions, fileNameRegexs)
	if !matched {
		return "", originalFileName, "", "", "", "", errors.New("Incorrect File Added For Flow - " + flowName)
	}
